                        ...
                        <div class="mb-3">
                            ...
                            <select name="region" id="region" class="form-control @error('region') is-invalid @enderror" required>
                                ...
                                @foreach($regions as $region)
                                    <option value="{{$region->id}}" {{ old('region', $customer->region->region_name ?? null) == $region->region_name ? 'selected' : '' }} @if($region->region_name == $customer->region->region_name ?? null) selected @endif >{{$region->region_name}}</option>
                                @endforeach
                            </select>
                            @error('region')
                            <div class="invalid-feedback">{{ $message }}</div>
